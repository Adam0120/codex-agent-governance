...
def adapter(role: dict[str, Any], base_hash: str) -> str:
    ...
    instruction = f"Execute `$govern-agent-system` in execute mode for registered role {name}. Before repository work, run `python3 \"$HOME/.agents/skills/govern-agent-system/scripts/agent_system.py\" profile --role {name} --cwd .` and follow the returned English contract. Do not spawn child agents. Return compact English parent-facing evidence only."
